...
```bash
cd /path/to/mlx_sam3
python scripts/label_interactive.py --annotation-file annotations/{session_dir}/{image_id}_annotations.json --output-dir annotations/{session_dir}
```

**Example:**

```bash
python scripts/label_interactive.py \
...
```bash
cd /path/to/mlx_sam3
python scripts/visualize_annotations.py annotations/{session_dir}/{image_id}_annotations.json
```
...
python scripts/visualize_annotations.py annotations/1_annotations.json
...
- Python 3.13+
- MLX and SAM3 model (installed via `pip install -e .`)
- PIL/Pillow
- NumPy
...
```bash
cd /path/to/mlx_sam3
uv pip install pycocotools pillow numpy
```
...
- All scripts use relative paths and should be run from the `mlx_sam3` directory
...
This project builds on top of the existing MLX SAM3 implementation:

**MLX SAM3: Native Apple Silicon Implementation**
